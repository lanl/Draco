...
//------------------------------------------------------------------------------------------------//
/*!
 * \brief Implementation for borydn methods used to solve a set of nonlinear equations.
 *
 * \sa broydn for more details
 *
 * \tparam Field A field type, such as double or complex.
 * \tparam Function_N_to_N A multifunctor type, representing a set of N functions in N variables.
 *         Each function returns the residual of the corresponding equation in the nonlinear system.
 *         \sa Function_N_to_N
 * \tparam Function_N_to_NN A multifunctor type, representing the Jacobian of a set of N functions
 ...
 *
 * \param[in] use_dvecfunc Tell the borydn solver to use functor/function-pointer/lambda operator
 *         \c dvecfunc instead of default to \c fdjac.
...
void broydn_impl(bool use_dvecfunc, std::vector<Field> &x, Function_N_to_N vecfunc,
...
void broydn(std::vector<Field> &x, const double /*STPMX*/, const Function_N_to_N &vecfunc,
  ...
  // unused lambda function needed by impl signature
  auto dvecfunc = [](std::vector<double> const & /*unused*/, std::vector<double> const & /*unused*/,
                     std::vector<double> & /*unused*/) {};
  double const min_lambda(0.0); // unused parameter needed by impl signature

  constexpr bool use_dvecfunc{false};
  broydn_impl(use_dvecfunc, x, vecfunc, dvecfunc, alf, min_lambda);
}
...
void broydn(std::vector<Field> &x, const double /*STPMX*/, Function_N_to_N vecfunc,
            Function_N_to_NN dvecfunc, const double alf, double const min_lambda) {
  constexpr bool use_dvecfunc{true};
  broydn_impl(use_dvecfunc, x, vecfunc, dvecfunc, alf, min_lambda);
}
